...
_start:
    ; read stdin into buf
    ; Call number 0 reads bytes from a file descriptor. Descriptor 0 is the keyboard/stdin.
    mov rax, 0          ; read
    mov rdi, 0          ; from stdin
    mov rsi, buf        ; where to store the input
    mov rdx, 4096       ; maximum bytes to read
    syscall             ; rax now holds the byte count
    mov r12, rax        ; r12 = number of bytes read (save it — rax gets reused by the next syscall)
...
.loop:
    ...
    ; An if/else is a cmp and a jump that skips the body, with a jmp over the else.
    ; First check: is al a lowercase letter (a-z)?
    cmp al, 'a'
...
.next:
    inc rcx              ; advance the counter — every loop must do this or it never ends
    jmp .loop            ; go test again

.done:
    ; write
    ; Call number 1 writes bytes to a file descriptor. Descriptor 1 is the screen/stdout.
    mov rax, 1          ; write
    mov rdi, 1          ; to stdout
    mov rsi, buf        ; address of the bytes
    mov rdx, r12        ; how many bytes — the original count read, unchanged
    syscall
